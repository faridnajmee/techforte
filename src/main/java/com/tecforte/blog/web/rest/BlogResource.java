...
public class BlogResource {
    ...
    public ResponseEntity<Void> deleteBlogByKeyword(@RequestParam String keywords) {
        log.debug("REST request to delete All Blog by keywords : {}", keywords);
        blogService.deleteBlogByKeyword(keywords);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, "All with Keyword")).build();
    }

    // Part2 Question 2
    @DeleteMapping("/blogs/{id}/clean")
    public ResponseEntity<Void> deleteSpecificBlogByKeyword(@PathVariable Long id,@RequestParam String keywords ) {
        log.debug("REST request to delete Specific Blog by ID :{} and keywords : {}", id , keywords);
        blogService.deleteSpecificBlogByKeyword(id, keywords);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, "All with Keyword and ID")).build();
    }
}
